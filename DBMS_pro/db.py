...
def cancel_tickets(ticket_ids):
    ...
    try:
        ...
        for tid, show_id, seat_id in rows:
            cursor.execute("UPDATE ShowSeat SET status='available' WHERE show_id = %s AND seat_id = %s AND status = 'reserved'", (show_id, seat_id))
...
def get_earnings_by_user_for_show(show_id):
    ...
    conn = db_connect()
    cursor = conn.cursor()
    # Count all confirmed tickets for the user for the show, but LEFT JOIN payments
    # because a single payment row may reference only one ticket (group payment).
    cursor.execute("""
        ...
        LEFT JOIN payment p ON p.ticket_id = t.ticket_id AND p.status = 'Success'
